...
def color_to_gradient(input_image):
...
#nput_image = cv2.imread("fixtures/full_sky.png")
from PIL import Image
img = Image.open('liza.jpg')
style = Image.open('out (6).png')
img = img.resize(style.size)
img.save('out.png')
...
fg = cv2.bitwise_or(input_image, input_image, mask = msl)
fg = cv2.cvtColor(fg, cv2.COLOR_BGR2RGB)
Image.fromarray(fg).save('o.jpg')

style.save('outstyle.png')
style = cv2.imread('outstyle.png')


msl = cv2.bitwise_not(msl)
style = cv2.bitwise_or(style,style, mask = msl)
style = cv2.cvtColor(style, cv2.COLOR_BGR2RGB)
Image.fromarray(style).save('k.jpg')
final = cv2.bitwise_or(fg,style)
Image.fromarray(final).save('final.jpg')


def get_result(content_image, styled_image):
    from PIL import Image
    img = Image.open(content_image)
    style = Image.open(styled_image)
    img = img.resize(style.size)
    img.save('./tmp/out.png')
    input_image = cv2.imread('./tmp/out.png')

    b1, b2 = detect_sky(input_image)
    msl = np.array(make_mask(b1, input_image))
    fg = cv2.bitwise_or(input_image, input_image, mask = msl)
    fg = cv2.cvtColor(fg,cv2.COLOR_BGR2RGB)
    Image.fromarray(fg).save('./tmp/otlad.jpg')
    style.save('./tmp/otladstyle.png')
    style = cv2.imread('./tmp/otladstyle.png')
    msl = cv2.bitwise_not(msl)
    style = cv2.bitwise_or(style,style, mask = msl)
    style = cv2.cvtColor(style, cv2.COLOR_BGR2RGB)
    Image.fromarray(style).save('./tmp/otlad_k.jpg')
    final = cv2.bitwise_or(fg,style)
    Image.fromarray(final).save('./result/final.jpg')
    print('Image saved in ./result/final.jpg')
