...
input_path = '/home/kirizhik/SkyGan/downloads/pixel sky'
...
print('Applying PCA...')
reduced_data = PCA(n_components=6).fit_transform(vec_mat)
...
for index, i in enumerate(sample_indices):
    file = files[i]
    filename = os.fsdecode(file)
    copyfile(input_path + '/' + filename, './i' + str(preds[index]) + '/' + filename)
